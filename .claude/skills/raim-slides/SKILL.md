...
`build_example.js` をコピーして中身を書き換える。
...
開発元は `~/workspace/raim/slide_template/`（出力 pptx/pdf 付き）。このスキルが配布版。配色やレイアウトを更新したら両者を同期する（このスキルを正本にしてよい）。
