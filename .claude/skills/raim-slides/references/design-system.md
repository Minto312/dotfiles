...
1. **1スライド1メッセージ**
2. **タイトル2層**：スライドタイトル（章名タグ）＋ **リード文＝そのスライド唯一のメッセージ**。リード文は体言止め・記号区切りでなく**文章として読める結論文**（〜45字・1行）にする
3. **So-What は ▶ で示す**（「示唆」の語は使わない）
4. **特徴→便益**、便益は可能なら定量（仮値は「(仮)」明示）
5. **視認性**：本文は詰めすぎない。詳細は Appendix へ

### 見出し・帯の役割分担（2026-07 確定。メッセージラインの重複禁止）

リード文を文章化すると、直下の濃緑 takeaway 帯が「2本目の結論ライン」になり役割が衝突する（実例: 同じ主張を2回書いたスライドが発生）。以下で統一する。

| 要素 | 役割 | 使い方 |
|---|---|---|
| 章番号タグ＋スライドタイトル（ヘッダー） | テーマ（何の話か） | 内容説明型の日本語。英語一単語タグ（Problem 等）は不可 |
| リード文（`leadText`・黒・大） | **唯一のメッセージ（結論）** | 1スライドに1本だけ |
| 濃緑 takeaway 帯（`addTitleBlock` の `takeaway`） | **原則使わない** | リード文と役割が重複するため。使うのは表など本文が仕様のみでメッセージ補強が別途必要な例外時 |
| グレー補助行（`sub` 相当・10.5pt textGray） | 仕様的な補足（成果物・判定基準・構成サマリ） | 主張はしない情報を降格して置く |
| 下部の ▶ 薄緑帯（summary/note） | 図表を受けた補足サマリ・So-What | 位置と見た目で別役割と分かる |

表紙は `coverTitle` を**左寄せ**（PowerPoint の title プレースホルダ既定は中央寄せのため `align:"left"` を明示済み）。`bigTitle` に run 配列を渡せばプロダクト名（大）＋タグライン（小）の2段構成にできる。
