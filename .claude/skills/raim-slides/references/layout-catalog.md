...
> ⚠️ **`takeaway`（リード文直下の濃緑帯）は原則使わない**（2026-07 確定）。リード文＝唯一のメッセージ（文章型の結論）とし、takeaway を足すと結論ラインが2本になって役割が衝突する。仕様的な補足（成果物・判定基準等）はグレー補助行（`sub` 相当、10.5pt textGray）へ降格し、So-What は下部の ▶ 薄緑帯で示す。詳細は design-system.md「見出し・帯の役割分担」。
